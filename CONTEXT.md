# RuSKKserv 開発引き継ぎコンテキスト (CONTEXT.md)
...
- **名称**: RuSKKserv（バイナリ・クレート名: `ruskkserv`）
- **リポジトリ**: `https://github.com/takahasee/RuSKKserv`
...
- 代わりに、**手動およびプリセットに基づく Seed ファイル (`~/.ruskkserv-frequency.json`)** を読み取り専用として使用します。
...
- **保存先**: `~/.ruskkserv-frequency.json`
- **組み込みプリセット**: `data/default-seed.json`（日常・ビジネス頻出の同音異義語コロケーションを約 420 エントリ網羅）。
- **`ruskkserv init-seed [--force]`**: 既存の個人頻度を壊さずに文脈プリセットを安全にマージ・初期化。
- **`ruskkserv import-user-dict <path> [--send-reload]`**: macSKK ユーザー辞書（`skk-jisyo.utf8`）から選択頻度をインポート。`--send-reload` 指定時はインポート完了後に `pkill -HUP -x ruskkserv` を実行してホットリロードをトリガーする（文脈共起が空の場合はプリセットも自動注入）。
- **SIGHUP ホットリロード**: `ruskkserv` プロセスが SIGHUP を受信すると、`~/.ruskkserv-frequency.json` を再起動なしで即座に再読み込みする。LaunchAgent による毎時自動インポートもホットリロードで反映される。
...
- **メカニズム**:
  ...
  - 送りあり復元は `RUSKKSERV_OKURI_EXPANSION=0` または `--okuri-expansion=false` で即座にロールバック可能。
...
- **Plist**:
  - `launchd/com.ruskkserv.skkserv.plist`（ラベル: `com.ruskkserv.skkserv`）
  - `launchd/com.ruskkserv.yaskkserv2.plist`（ラベル: `com.ruskkserv.yaskkserv2`）
  - `launchd/com.ruskkserv.import-user-dict.plist`（ラベル: `com.ruskkserv.import-user-dict`）
- **起動スクリプト**:
  - `scripts/wait-and-run-ruskkserv.sh`: azooKey (:1180) と yaskkserv2 (:1179) の待機後に `ruskkserv` を起動。
- **管理スクリプト**:
  - `scripts/install-launchd.sh [install|uninstall|status]`
- **設定ファイル**:
  - `launchd/config.env`
- **ログ**: `~/Library/Logs/ruskkserv/`
